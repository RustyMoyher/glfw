...
static CVReturn nsglDisplayLinkCallback(CVDisplayLinkRef displayLink, const CVTimeStamp* now, const CVTimeStamp* outputTime, CVOptionFlags flagsIn, CVOptionFlags* flagsOut, void* userContext)
{
...
#undef setAttrib
    ...
    // instead.
    int swapInterval = 0;
    [window->context.nsgl.object setValues:&swapInterval forParameter:NSOpenGLContextParameterSwapInterval];

    CVDisplayLinkCreateWithActiveCGDisplays(&window->context.nsgl.displayLink);
    CVDisplayLinkSetOutputCallback(window->context.nsgl.displayLink, &nsglDisplayLinkCallback, (void*)&window->context.nsgl);
...
void _glfwUpdateDisplayLinkNSGL(_GLFWwindow* window) {
    CVDisplayLinkSetCurrentCGDisplayFromOpenGLContext(window->context.nsgl.displayLink, [window->context.nsgl.object CGLContextObj], [window->context.nsgl.pixelFormat CGLPixelFormatObj]);
}
